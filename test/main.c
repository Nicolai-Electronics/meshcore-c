...
unsigned char new_advertisement[] = {0x12, 0x00, 0x43, 0x8d, 0x81, 0x94, 0x70, 0xeb, 0x33, 0x26, 0x4d, 0xc8, 0xed, 0xde, 0x6a, 0x92, 0x10, 0x2c, 0xcd, 0x76,
                                     0x14, 0xef, 0x13, 0x7f, 0x31, 0xf5, 0x11, 0x3c, 0x0e, 0xc3, 0xef, 0xdd, 0xe8, 0x28, 0xc0, 0x09, 0xbd, 0x85, 0x3b, 0x13,
                                     0x23, 0x16, 0xc8, 0xc0, 0xf2, 0xa5, 0x61, 0x86, 0xa6, 0xe2, 0x0c, 0x86, 0x88, 0x29, 0x22, 0x5c, 0x8d, 0xd9, 0xf7, 0x1e,
                                     0x73, 0xc4, 0xae, 0x85, 0x11, 0x43, 0x56, 0xce, 0xfd, 0xe6, 0x3a, 0xa1, 0xa6, 0x1a, 0x49, 0xcb, 0x48, 0x23, 0x50, 0x3d,
                                     0x17, 0xc2, 0x87, 0x5a, 0x44, 0x82, 0x65, 0x57, 0x9a, 0x6e, 0xb5, 0x6d, 0x11, 0xe9, 0x7b, 0x64, 0x62, 0x76, 0x7f, 0x94,
                                     0x3c, 0x5e, 0x5b, 0xee, 0xad, 0xd3, 0x3c, 0xe9, 0xa2, 0xc5, 0xb9, 0x9c, 0x45, 0xea, 0x33};
    ...
    if (meshcore_deserialize(packet, packet_size, &message) >= 0) {
        ...
        if (message.type == MESHCORE_PAYLOAD_TYPE_ADVERT) {
            ...
            if (meshcore_advert_deserialize(message.payload, message.payload_length, &advert) >= 0) {
                ...
                memcpy(&verification_data[verification_data_size], &message.payload[MESHCORE_PUB_KEY_SIZE + sizeof(uint32_t) + MESHCORE_SIGNATURE_SIZE],
                       message.payload_length - MESHCORE_PUB_KEY_SIZE - sizeof(uint32_t) - MESHCORE_SIGNATURE_SIZE);
                verification_data_size += message.payload_length - MESHCORE_PUB_KEY_SIZE - sizeof(uint32_t) - MESHCORE_SIGNATURE_SIZE;
